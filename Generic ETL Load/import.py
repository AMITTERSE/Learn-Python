...
def mysql_to_gcs():
    ...
    for idx, mysql_database in enumerate(config.mysql['dbs']):
        ...
        mysql_connection_url = 'mysql+mysqldb://'+mysql_database['user']+':'+mysql_database['pw']+'@'+mysql_database['host']+"/"+mysql_database['name']
        mysql_engine = create_engine(mysql_connection_url)
        # Change Needed for SAP HANA
        mysql_table_names = mysql_engine.table_names()
        # Restrict the List of Tables
